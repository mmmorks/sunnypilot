  ...
  for msg in sorted(lr1, key=lambda msg: msg.logMonoTime) if MULTI_FILE else tqdm(sorted(lr1, key=lambda msg: msg.logMonoTime)):
    ...
    try:
      ...
      if msg.which() == 'carState':
        ...
        s.yaw_rate = msg.livePose.angularVelocityCalibrated.value[2]
        s.pitch = msg.livePose.orientationNED.value[1]
...
def load(path, route=None, preprocess=False, dongleid=False, outpath=""):
  ...
  if route:
    ...
    try:
      ...
      lr = LogReader(r.log_paths(), sort_by_time=True)
      data = collect(lr)
        ...
        if "realdata" in path or (preprocess and dongleid):
          ...
          for filename in tqdm(filenames, desc="Preparing fit data from rlogs"):
            if len(filename.split('--')) == 3 and f"{dongle_id}|{filename}.lat" not in latsegs:
              with tempfile.TemporaryDirectory() as d:
                ...
                elif os.path.exists(os.path.join(path,filename,"rlog.zst")):
                  tmpzst = os.path.join(d,f"{dongle_id}_{filename}--rlog.zst")
                  shutil.copy(os.path.join(path,filename,"rlog.zst"),tmpzst)

                seg_num = f"{dongle_id}|{filename}".split('--')[2]
                try:
                  route='--'.join(f"{dongle_id}|{filename}".split('--')[:2])
                  r = Route(route, data_dir=d)
                  lr = LogReader([lp for lp in r.log_paths() if lp])
                  data1 = collect(lr)
                  if len(data1):
          ...
          for filename in os.listdir(outpath):
            if ext in filename.split("/")[-1]:
              p1 = filename.replace(outpath, path).replace('.lat','--rlog.zst').replace('|','_')
              p2 = filename.replace(outpath, path).replace('.lat','--rlog.zst')
              ...
              if filename not in latsegs:
                ...
                with tempfile.TemporaryDirectory() as d:
                  try:
                    ...
                    r = Route(route, data_dir=d)
                    logs = [x for x in r.log_paths() if x is not None]
                    lr = LogReader(logs, sort_by_time=True)
                    data1 = collect(lr)
          ...
          if preprocess:
            for filename in filenames:
                latsegs.add(filename.replace(outpath, path).replace('.lat','--rlog.zst').replace('|','_'))
                latsegs.add(filename.replace(outpath, path).replace('.lat','--rlog.zst'))
            with open(rlog_log_path, 'w') as rll:
              for ls in sorted(list(latsegs)):
                rll.write(f"\n{ls}")
